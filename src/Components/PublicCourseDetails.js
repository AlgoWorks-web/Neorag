...
function PublicCourseDetails() {
    ...
    const handleEnroll = () => {
        const isLoggedIn = !!localStorage.getItem('studentUser');
        if (isLoggedIn) {
            navigate(`/student/useragreement/${courseId}`);
        } else {
            toast.warn('Please login or sign up to enroll in this course');
    ...
    return (
        <div className="bg-gray-50 py-24 px-4 md:px-12">

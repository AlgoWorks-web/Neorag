...
const MaterialsList = () => {
  ...
  // ✅ ADDED: Helper function to safely get course info (handles both old and new data structures)
  ...
  // ✅ UPDATED: Fetch enrolled courses with proper data structure handling
  const fetchMyCourses = async () => {
    try {
      ...
      console.log('=== MATERIALS API Response Debug ===');
      ...
      if (data.success) {
        ...
        // ✅ UPDATED: Auto-select first course using new data structure
  ...
  // ✅ UPDATED: Fetch materials with better URL construction
  const fetchCourseMaterials = async (courseId) => {
    if (!courseId) return;

    console.log('=== FETCH MATERIALS DEBUG ===');
    ...
    try {
      ...
      const materialsUrl = `https://hydersoft.com/api/enrolledstudent/course/${courseId}/materials`;
      console.log('Materials API URL:', materialsUrl);
      ...
      if (data.success) {
        ...
        setError(data.error || data.message || 'Failed to fetch materials');
        setMaterials([]);
      }
    } catch (err) {
  ...
  // ✅ ADDED: Debug logging for courses data
  useEffect(() => {
  ...
  // ✅ UPDATED: Course selection handler
  const handleCourseChange = (e) => {
    const courseId = e.target.value;
    console.log('=== COURSE SELECTION DEBUG ===');
    console.log('Selected course ID:', courseId);
    console.log('Selected course type:', typeof courseId);
  ...
  const handlePreview = (material) => {
    try {
      ...
      let previewUrl = material.preview_url;
      if (!previewUrl.includes('student_id=')) {
        const separator = previewUrl.includes('?') ? '&' : '?';
        previewUrl = `${previewUrl}${separator}student_id=${studentUser.id}`;
      }

      const separator2 = previewUrl.includes('?') ? '&' : '?';
      previewUrl = `${previewUrl}${separator2}preview=true`;

      // ✅ CLEANEST: Just open, no detection
      window.open(previewUrl, '_blank', 'noopener,noreferrer');

      console.log('✅ Preview requested - opened in new tab');

    } catch (err) {
      console.error('Error previewing material:', err);
      setError('Failed to preview material.');
    }
  };
      ...
      <div className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            {/* ✅ UPDATED: Course Selector with new data structure */}

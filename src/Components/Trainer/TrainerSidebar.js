...
const TrainerSidebar = ({ sidebarOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`bg-gradient-to-b from-blue-900 via-blue-800 to-blue-900 text-white w-64 fixed inset-y-0 left-0 z-40 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        ...
        <nav className="px-3 pb-6 space-y-2">
          {menuItems.map((item, index) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `group relative flex items-center space-x-3 py-3 px-4 rounded-xl transition-all duration-200 ${isActive
            ...
            >
              {/* Active Indicator */}
              <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-1 h-8 bg-blue-300 rounded-r-full transition-all duration-200 ${({ isActive }) => isActive ? 'opacity-100' : 'opacity-0'
                }`} />
